  ...
  if (button_state == 1)
  {
    ...
    switch (state) {
      ...
      case 7: // keep displaying third message until time expires
        if ((currentMillis - motionStartedAt) > 8000) {
          state = 8;
        }
        break;
...
void firstMessage() {
  ...
  display.println("Something seems wrong.");
  display.display();
}
...
void secondMessage() {
  ...
  display.setCursor(0, 0);            // Start at top-left corner
  display.println("Message: ");
  display.println("Can't find safety - abort.");
  display.display();
}
...
void thirdMessage() {
  ...
  display.println("Power source seems incompatible.");
  display.display();
}
...
void fourthMessage() {
  ...
  display.setCursor(0, 0);            // Start at top-left corner
  display.println("Error: ");
  display.println("Stuck in an infinite loop.");
  display.display();
}
